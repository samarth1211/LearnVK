...
// Buffers
VkBuffer g_vlkInputBuffer = VK_NULL_HANDLE;
VkBuffer g_vlkOutputBuffer = VK_NULL_HANDLE;
VkDeviceMemory g_vlkInputBufferMemory = VK_NULL_HANDLE;
VkDeviceMemory g_vlkOutputBufferMemory = VK_NULL_HANDLE;

uint32_t InputData[NUM_ELEMENTS]; // cpu to gpu
...
int main(int argc, char **argv, char **envp)
{
    ...
    inputBufferCreateInfo.pNext = nullptr;
    inputBufferCreateInfo.size = sizeof(uint32_t) * NUM_ELEMENTS;
    inputBufferCreateInfo.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;

    vlkRes = vkCreateBuffer(g_vlkLogicalDevice, &inputBufferCreateInfo, nullptr, &g_vlkInputBuffer);
    if (vlkRes != VK_SUCCESS)
    {
        std::cout << "vkCreateBuffer(g_vlkInputBuffer) FAILED" << std::endl;
        CleanUp();
        return EXIT_FAILURE;
    }
    else
    {
        std::cout << "vkCreateBuffer(g_vlkInputBuffer) Complete" << std::endl;
    }

    VkMemoryRequirements memoryRequirements{};
    vkGetBufferMemoryRequirements(g_vlkLogicalDevice, g_vlkInputBuffer, &memoryRequirements);

    VkMemoryAllocateInfo memAllocInfoInputBuffer{};
    memAllocInfoInputBuffer.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    memAllocInfoInputBuffer.pNext = nullptr;
    memAllocInfoInputBuffer.allocationSize = memoryRequirements.size;
    memAllocInfoInputBuffer.memoryTypeIndex = FindMemoryIndexByType(memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);

    vlkRes = vkAllocateMemory(g_vlkLogicalDevice, &memAllocInfoInputBuffer, nullptr, &g_vlkInputBufferMemory);
    if (vlkRes != VK_SUCCESS)
    {
        std::cout << "vkAllocateMemory(g_vlkInputBufferMemory) FAILED" << std::endl;
        CleanUp();
        return EXIT_FAILURE;
    }

    vlkRes = vkBindBufferMemory(g_vlkLogicalDevice, g_vlkInputBuffer, g_vlkInputBufferMemory, 0);
    if (vlkRes != VK_SUCCESS)
    {
        std::cout << "vkBindBufferMemory(g_vlkInputBuffer, g_vlkInputBufferMemory) FAILED" << std::endl;
        CleanUp();
        return EXIT_FAILURE;
    }
    ...
    descriptorBuffers[0].buffer = g_vlkInputBuffer;
    descriptorBuffers[0].offset = 0;
    descriptorBuffers[0].range = sizeof(uint32_t) * NUM_ELEMENTS;

    descriptorBuffers[1].buffer = g_vlkOutputBuffer;
    ...
    writeDecriptorSet.descriptorCount = 2;
    ...
    vkCmdDispatch(g_vlkCommandBuffer, 6, 1, 1);
    ...
    vlkRes = vkMapMemory(g_vlkLogicalDevice, g_vlkInputBufferMemory, 0, sizeof(uint32_t) * NUM_ELEMENTS, 0, &inputAddress);
    if (vlkRes != VK_SUCCESS)
    {
        std::cout << "vkMapMemory(g_vlkInputBufferMemory) FAILED" << std::endl;
        CleanUp();
        return EXIT_FAILURE;
    }
    else
    {
        std::cout << "vkMapMemory(g_vlkInputBufferMemory) Complete" << std::endl;
    }

    memcpy(inputAddress, InputData, sizeof(uint32_t) * NUM_ELEMENTS);

    vkUnmapMemory(g_vlkLogicalDevice, g_vlkInputBufferMemory);
...
void CleanUp()
{
    ...
    if (g_vlkLogicalDevice)
    {
        ...
        if (g_vlkInputBuffer)
        {
            vkDestroyBuffer(g_vlkLogicalDevice, g_vlkInputBuffer, nullptr);
            g_vlkInputBuffer = VK_NULL_HANDLE;
            vkFreeMemory(g_vlkLogicalDevice, g_vlkInputBufferMemory, nullptr);
            g_vlkInputBufferMemory = VK_NULL_HANDLE;
