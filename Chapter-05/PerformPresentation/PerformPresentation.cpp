...
#define NUM_FRAMES 7 // 8 is max images in Vulkan currently
...
VkClearColorValue g_vlkClearColor = {0.15f, 0.26f, 0.66f, 1.0f}; // R, G, B, A

VkSemaphore g_vlkNextImageAvailableSemaphore = VK_NULL_HANDLE;
VkSemaphore g_vlkRenderCompleteSemaphore = VK_NULL_HANDLE;
#pragma endregion
...
bool Resize(HWND hWnd, int iWidth, int iHeight)
{
	...
	if (g_vlkDevice)
	{
		...
		if (g_vlkRenderCompleteSemaphore)
		{
			vkDestroySemaphore(g_vlkDevice, g_vlkRenderCompleteSemaphore, nullptr);
			g_vlkRenderCompleteSemaphore = VK_NULL_HANDLE;
		}

		if (g_vlkNextImageAvailableSemaphore)
		{
			vkDestroySemaphore(g_vlkDevice, g_vlkNextImageAvailableSemaphore, nullptr);
			g_vlkNextImageAvailableSemaphore = VK_NULL_HANDLE;
		}
		...
		for (size_t iIdx = 0; iIdx < presentModesList.size(); iIdx++)
		{
			...
			if (presentModesList[iIdx] == VK_PRESENT_MODE_FIFO_KHR) // with v-sync
			{
		...
		swapChainCreateInfo.minImageCount = ((NUM_FRAMES < surfaceCapabilities.maxImageCount) && (NUM_FRAMES >= surfaceCapabilities.minImageCount)) ? NUM_FRAMES : surfaceCapabilities.maxImageCount - 1;
		...
		if (g_PresentationQFamilyIdx != g_CommandQFamilyIdx)
		{
			uint32_t queueFamilyIndices[2] = {(uint32_t)g_CommandQFamilyIdx, (uint32_t)g_PresentationQFamilyIdx};
		...
		vkCreateSemaphore(g_vlkDevice, &semaphoreCreateInfo, nullptr, &g_vlkNextImageAvailableSemaphore);
		if (vlkRes != VK_SUCCESS)
		{
			WRITE_IN_LOG_FILE("Failed to Create Semaphore for Acquire Next Image\n");
			return false;
		}

		vkCreateSemaphore(g_vlkDevice, &semaphoreCreateInfo, nullptr, &g_vlkRenderCompleteSemaphore);
		if (vlkRes != VK_SUCCESS)
		{
			WRITE_IN_LOG_FILE("Failed to Create Semaphore for Acquire Next Image\n");
			return false;
...
void Render(void)
{
	...
	// For presentation take the next image
	uint32_t iCurrentImageIndex;
	vkAcquireNextImageKHR(g_vlkDevice, g_vlkSwapChain, UINT32_MAX, g_vlkNextImageAvailableSemaphore, VK_NULL_HANDLE, &iCurrentImageIndex);
	...
	cmdCommandBufferBeginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
	// VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT re-use of command buffers
	...
	presentToClearBarrier.srcQueueFamilyIndex = g_PresentationQFamilyIdx;
	presentToClearBarrier.dstQueueFamilyIndex = g_PresentationQFamilyIdx;
	presentToClearBarrier.image = g_vlkSwapChainImages[iCurrentImageIndex];
	...
	clearToPresentBarrier.newLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; // It means that the Image is reaady for presentation.
	clearToPresentBarrier.srcQueueFamilyIndex = g_PresentationQFamilyIdx;
	clearToPresentBarrier.dstQueueFamilyIndex = g_PresentationQFamilyIdx;
	clearToPresentBarrier.image = g_vlkSwapChainImages[iCurrentImageIndex];
	...
	// convert the view of image from one state to presentable one.
	/*VkImageMemoryBarrier imgMemBarrier{};
	imgMemBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
	imgMemBarrier.pNext = nullptr;
	imgMemBarrier.srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
	imgMemBarrier.dstAccessMask = VK_ACCESS_MEMORY_READ_BIT;
	imgMemBarrier.oldLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
	imgMemBarrier.newLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
	imgMemBarrier.srcQueueFamilyIndex = g_PresentationQFamilyIdx;
	imgMemBarrier.dstQueueFamilyIndex = g_PresentationQFamilyIdx;
	...
	vkCmdPipelineBarrier(g_vlkPresentCommandBuffer,
	...
	vkQueuePresentKHR(g_vlkPresentationQueue, &presentInfo);*/

	vkCmdPipelineBarrier(g_vlkPresentCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &presentToClearBarrier);

	vkCmdClearColorImage(g_vlkPresentCommandBuffer, g_vlkSwapChainImages[iCurrentImageIndex], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &g_vlkClearColor, 1, &imageSubResourceRange);

	vkCmdPipelineBarrier(g_vlkPresentCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, nullptr, 0, nullptr, 1, &clearToPresentBarrier);
	...
	presentCommandSubmitInfo.waitSemaphoreCount = 1;
	presentCommandSubmitInfo.pWaitSemaphores = &g_vlkNextImageAvailableSemaphore;
	presentCommandSubmitInfo.signalSemaphoreCount = 1;
	presentCommandSubmitInfo.pSignalSemaphores = &g_vlkRenderCompleteSemaphore;
	VkPipelineStageFlags waitDstStageMask = VK_PIPELINE_STAGE_TRANSFER_BIT;
	presentCommandSubmitInfo.pWaitDstStageMask = &waitDstStageMask;
	...
	presentInfo.pWaitSemaphores = &g_vlkRenderCompleteSemaphore;
	presentInfo.swapchainCount = 1;
	presentInfo.pSwapchains = &g_vlkSwapChain;
	presentInfo.pImageIndices = &iCurrentImageIndex;
	presentInfo.pResults;
	vkQueuePresentKHR(g_vlkPresentationQueue, &presentInfo);
...
int Initialize(HWND hWnd)
{
	...
	for (size_t iIdx = 0; iIdx < instanceLayerList.size(); iIdx++)
	{
		...
		if (strcmp(instanceLayerList[iIdx].layerName, "VK_LAYER_LUNARG_monitor") == 0)
		{
			instanceLayerNames.push_back(instanceLayerList[iIdx].layerName);
		}
